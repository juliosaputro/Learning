...
import materiempat from "../../const/materiempat";

const { width } = Dimensions.get("screen");
const cardWidth = width / 2 - 20;

const BabEmpatScreen = ({ navigation }) => {

  const Card = ({ materiempat }) => {
    return (
      <TouchableHighlight
        underlayColor={COLORS.white}
        activeOpacity={0.9}
        onPress={() => navigation.navigate("DetailScreen", materiempat)}
      >
        <View style={styles.card}>
          <View style={{ alignItems: "center", top: 0 }}>
            ...
            <Text style={{ fontSize: 48, fontWeight: "bold", color:COLORS.primary}}>{materiempat.id}</Text>
            </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      ...
      <FlatList
        ...
        numColumns={2}
        data={materiempat}
        renderItem={({ item }) => <Card materiempat={item} />}
      />
    </SafeAreaView>
  );
};

export default BabEmpatScreen;
