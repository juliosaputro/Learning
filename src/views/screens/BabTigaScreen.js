...
import materitiga from "../../const/materitiga";

const { width } = Dimensions.get("screen");
const cardWidth = width / 2 - 20;

const BabTigaScreen = ({ navigation }) => {

  const Card = ({ materitiga }) => {
    return (
      <TouchableHighlight
        underlayColor={COLORS.white}
        activeOpacity={0.9}
        onPress={() => navigation.navigate("DetailScreen", materitiga)}
      >
        <View style={styles.card}>
          <View style={{ alignItems: "center", top: 0 }}>
            ...
            <Text style={{ fontSize: 48, fontWeight: "bold", color:COLORS.primary}}>{materitiga.id}</Text>
            </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      ...
      <FlatList
        ...
        numColumns={2}
        data={materitiga}
        renderItem={({ item }) => <Card materitiga={item} />}
      />
    </SafeAreaView>
  );
};

export default BabTigaScreen;
